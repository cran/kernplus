cut.pts <- function(cov, circ = TRUE, .n.reg) {
  ...
  if (length(id.cand) > 0) {
    ...
    if (circ & brks[1] <= 0 & brks[length(brks)] >= 360) {
      if (length(id.cand) == 1)
        loc.cut <- (brks[id.cand] + brks[id.cand + 1])/2 else {
        ...
        if (any(vals == 1)) {
          ...
          if (id.pos == 1)
          id.rng <- c(id.cand[1], id.cand[sum(lens[1:(id.pos)]) + 1]) else id.rng <- id.cand[c(sum(lens[1:(id.pos - 1)]), sum(lens[1:(id.pos)])) +
          1]
    ...
    gmm <- mixtools::normalmixEM(cov, k = .n.reg)
    if (gmm$mu[1] < gmm$mu[2])
      id.clust <- which(gmm$posterior[, 1] > gmm$posterior[, 2]) else id.clust <- which(gmm$posterior[, 1] < gmm$posterior[, 2])
